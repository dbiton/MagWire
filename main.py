...
def extend_wire_from_anchor(wire_segments):
    # Extend the wire by adding a new segment at the anchor
    first_segment_body = wire_segments[0]
    new_segment_body = pymunk.Body(mass=0.1, moment=10)
    new_segment_body.position = (
        anchor_point[0] - segment_length,  # Place new segment to the left of the anchor
        anchor_point[1],
    )
    
    new_segment_shape = pymunk.Segment(
        new_segment_body, (0, 0), (segment_length, 0), segment_length / 32
    )
    new_segment_shape.elasticity = 0.5
    space.add(new_segment_body, new_segment_shape)
    
    # Create joint between the new segment and the old first segment
    joint = pymunk.PinJoint(new_segment_body, first_segment_body, (segment_length, 0), (0, 0))
    space.add(joint)
    
    # Add a rotary spring to simulate stiffness
    spring = pymunk.DampedRotarySpring(
        new_segment_body, first_segment_body, rest_angle=0, stiffness=wire_rigidity, damping=10
    )
    space.add(spring)
    
    # Attach the new first segment to the anchor point
    anchor_joint = pymunk.PinJoint(space.static_body, new_segment_body, anchor_point, (0, 0))
    space.add(anchor_joint)
    
    # Remove the old anchor joint
    for joint in space.constraints:
        if isinstance(joint, pymunk.PinJoint) and joint.a == space.static_body and joint.b == first_segment_body:
            space.remove(joint)
            break

    # Insert the new segment at the beginning of the wire segment list
    wire_segments.insert(0, new_segment_body)
